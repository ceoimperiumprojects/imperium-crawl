 ...
 *   --export-csv      Export JSONL to CSV files
 */
...
function parseArgs() {
  ...
  if (!stats && !exportCsvFlag && phase === 1 && !excelPath) {
    console.error(
  ...
  return { excelPath, limit, resume, phase, concurrency, stats, exportCsvFlag };
}
...
async function main() {
  const { excelPath, limit, resume, phase, concurrency, stats, exportCsvFlag } =
    parseArgs();
